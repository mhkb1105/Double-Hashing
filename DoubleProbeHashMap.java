...
public class DoubleProbeHashMap<K, V> extends ProbeHashMap<K, V> {
	...
	protected void resize(int newCap) {
		super.resize(newCap);
		q = selectSecondaryHashPrime(capacity);
	}
	...
	protected int findSlot(int h1, K k) {
		...
		do {
			...
			else if (table[current].getKey().equals(k)) {
				return current;                                 
			}
			
			current = (h1 + secondaryHashValue(k)*x) % capacity; x++;                      
		} while (current != h1 && !secHash);                            
		return -slot -1; 
